...
def mentions_counts_by_question(
    ...
    Wartości są ważone (weights) i zaokrąglone do int.
    """
    w = np.asarray(weights, dtype=float).reshape(-1)
    idx = {a: i for i, a in enumerate(ARCHETYPES)}
...
"""
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      function setChartTitles(a){
        ...
        if (t1) t1.textContent = (getMode() === "values" ? "% wskazań wartości " : "% wskazań archetypu ") + labelForArche(a);
...
<div class="tabs">
  ...
  <div class="panel pR">
    ...
    <div class="grid" style="margin-top:16px;">
      <div class="card">
        <h3 id="filter_title_pct"><span class="mode-arche">% wskazań archetypu</span><span class="mode-values">% wskazań wartości</span></h3>
  ...
  <div class="panel pT">
    ...
    <div class="small">
      ...
      A liczone „w ramach głosów dla <span class="mode-arche">archetypu</span><span class="mode-values">wartości</span>” (zgodnie z logiką par).
...
def main() -> None:
    ...
    df_mentions = total_mentions_counts(A, A_cols, b1, b2, d13, weights)
    ...
    # ---------- A: pełne % (wszystkie archetypy) ----------
    w = np.asarray(weights, dtype=float).reshape(-1)

    wins_A = {a: 0.0 for a in ARCHETYPES}
    votes_A = {a: 0.0 for a in ARCHETYPES}
    ...
    for j, (_pid, left_arch, right_arch) in enumerate(A_PAIRS):
        ...
        pair_votes = float(w[m].sum())
        votes_A[left_arch] += pair_votes
        votes_A[right_arch] += pair_votes

        wins_A[left_arch] += float(w[m & (v < A_SCALE_CENTER)].sum())
        wins_A[right_arch] += float(w[m & (v > A_SCALE_CENTER)].sum())

    df_A_pct_all = pd.DataFrame({
        "archetyp": list(wins_A.keys()),
        "%": [
            (wins_A[a] / votes_A[a] * 100.0) if votes_A[a] > 0 else np.nan
            for a in wins_A.keys()
        ],
    })
